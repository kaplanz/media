...
- `/games/owned`: releases, consoles, and accessories, discriminated by `kind`
  ...
  shorthand for a list of one. `sha1=<hex>` narrows the list to the items
...
filtered by `owned=<id>`, `sha1=<hex>`, or `q=<text>` against the filename.
...
Uploads take a raw body, with the filename and dump date as optional query
