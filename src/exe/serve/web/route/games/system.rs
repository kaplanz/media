...
use media::games::system::{Body, System};
...
use crate::schema::games_system as t;
...
#[utoipa::path(
    ...
    tag = "games/system",
    params(Params),
    responses((status = 200, body = Vec<System>)),
)]
async fn list(
    State(db): State<Pool>,
    Query(params): Query<Params>,
) -> Result<Json<Vec<System>>, Error> {
...
#[utoipa::path(
    ...
    tag = "games/system",
    params(("id" = Uuid, Path)),
    responses((status = 200, body = System), (status = 404)),
)]
async fn fetch(State(db): State<Pool>, Path(id): Path<Uuid>) -> Result<Json<System>, Error> {
    let mut conn = db::get_conn(&db).await.map_err(Error::from)?;
    let row = t::table
