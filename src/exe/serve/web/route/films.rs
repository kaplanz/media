...
#[utoipa::path(get, path = "/", tag = "films",
    params(Params),
...
async fn list(
    ...
    let mut sql = String::from(
        "SELECT films.id, tmdb, title, year, rated, \
         ...
         FROM films JOIN media ON media.id = films.id",
    );
    ...
    if params.tag.is_some() {
        conds.push(
            "EXISTS (SELECT 1 FROM tags \
             WHERE tags.media = films.id AND tags.label = ?)",
        );
    }
    if !conds.is_empty() {
        sql.push_str(" WHERE ");
    ...
    let all_tags = sqlx::query_as::<_, (Uuid, String)>(
        "SELECT tags.media, tags.label \
         FROM tags JOIN films ON films.id = tags.media \
         ORDER BY tags.label",
    )
    .fetch_all(&db)
    .await
...
#[utoipa::path(post, path = "/", tag = "films",
    request_body = Body,
