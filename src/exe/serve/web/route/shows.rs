...
async fn list(
    ...
    let rows: Vec<(Show, i64, i64)> = {
        let q = match (
            ...
            (Sort::Rating, Order::Asc) => query.order_by(shows::rating.asc()),
            (Sort::Rating, Order::Desc) => query.order_by(shows::rating.desc()),
    ...
    let records = rows
        ...
        .map(|(show, created, updated)| {
            ...
            let logs = logs.remove(&show.id).unwrap_or_default();
    ...
    if !body.is_empty() {
        let n = diesel::update(shows::table.filter(shows::id.eq(uid)))
            .set((
                ...
                body.rating.map(|v| shows::rating.eq(v)),
            ))
            .execute(&mut conn)
            .await
            .inspect_err(|err| tracing::error!("{err}"))
