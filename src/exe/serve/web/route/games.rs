...
#[utoipa::path(get, path = "/", tag = "games",
    params(Params),
    ...
    if params.tag.is_some() {
        conds.push(
            "EXISTS (SELECT 1 FROM tags \
             WHERE tags.media = games.id AND tags.label = ?)",
        );
    }
    if !conds.is_empty() {
        sql.push_str(" WHERE ");
...
#[utoipa::path(get, path = "/{id}", tag = "games",
    params(("id" = Uuid, Path)),
...
#[utoipa::path(post, path = "/", tag = "games",
    request_body = Body,
...
#[utoipa::path(put, path = "/{id}", tag = "games",
    params(("id" = Uuid, Path)),
    request_body = Body,
...
#[utoipa::path(delete, path = "/{id}", tag = "games",
    params(("id" = Uuid, Path)),
    responses((status = 204), (status = 404)))]
