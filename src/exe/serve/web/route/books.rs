...
use media::book::{Body, Book, Patch};
use media::logs::Log;
use media::{Item, Meta};
...
use crate::schema::{books, media as m, tags};
...
pub fn router() -> Router<Pool> {
    Router::new()
        ...
        .layer(Extension(media::Kind::Book))
}
...
async fn list(
    ...
    let mut query = books::table
        .inner_join(m::table)
        .select((books::all_columns, m::created, m::updated))
        .into_boxed();

    // Apply filters
    if let Some(q) = params.q {
        query = query.filter(books::title.like(format!("%{q}%")));
    }
    if let Some(tag) = params.tag {
        query = query.filter(diesel::dsl::exists(
            tags::table
                .filter(tags::media.eq(books::id))
                .filter(tags::label.eq(tag)),
        ));
    }

    // Sort and paginate
    let rows: Vec<(Book, i64, i64)> = {
        let q = match (
            params.sort.unwrap_or_default(),
            params.order.unwrap_or_default(),
        ) {
            (Sort::Title, Order::Asc) => query.order_by(books::title.asc()),
            (Sort::Title, Order::Desc) => query.order_by(books::title.desc()),
    ...
    // Load tags
    let ids: Vec<DbUuid> = rows.iter().map(|(b, _, _)| b.id.into()).collect();
    let mut tags = super::tags::load_tags_for(&mut conn, &ids).await?;
    let mut logs = super::logs::load_logs_for(&mut conn, &ids).await?;

    let records = rows
        .into_iter()
        .map(|(book, created, updated)| {
            let tags = tags.remove(&book.id).unwrap_or_default();
            let logs = logs.remove(&book.id).unwrap_or_default();
...
async fn fetch(State(db): State<Pool>, Path(id): Path<Uuid>) -> Result<Json<Record>, Error> {
    ...
    // Load item
    let (book, created, updated) = books::table
        .inner_join(m::table)
        .select((books::all_columns, m::created, m::updated))
        .filter(books::id.eq(uid))
        .first::<(Book, i64, i64)>(&mut conn)
        .await
        .optional()
        .map_err(Error::from)?
    ...
    conn.transaction(|conn| {
        async move {
            diesel::insert_into(m::table)
                .values((m::id.eq(uid), m::kind.eq("book")))
                .execute(conn)
                .await?;
            diesel::insert_into(books::table)
                .values((
                    ...
                    books::hcid.eq(body.hcid),
                    books::title.eq(&body.title),
                    books::cover.eq(&body.cover),
                    books::about.eq(&body.about),
                    books::color.eq(&body.color),
                ))
                .execute(conn)
                .await?;
...
async fn update(
    ...
    let mut conn = db::get_conn(&db).await.map_err(Error::from)?;
    let uid = DbUuid::from(id);
    let n = diesel::update(books::table.filter(books::id.eq(uid)))
        .set((
            books::isbn.eq(&body.isbn),
            books::hcid.eq(body.hcid),
            books::title.eq(&body.title),
...
async fn modify(
    ...
    if !body.is_empty() {
        let n = diesel::update(books::table.filter(books::id.eq(uid)))
            .set((
                body.isbn.map(|v| books::isbn.eq(v)),
                body.hcid.map(|v| books::hcid.eq(v)),
                body.title.map(|v| books::title.eq(v)),
                body.cover.map(|v| books::cover.eq(v)),
                body.about.map(|v| books::about.eq(v)),
                body.color.map(|v| books::color.eq(v)),
            ))
            .execute(&mut conn)
            .await
            .inspect_err(|err| tracing::error!("{err}"))
