...
/** Which digest a hash names, by its width in hexadecimal characters. */
const WIDTHS: Record<number, (typeof DIGESTS)[number] | undefined> = {
    8: "crc32",
    32: "md5",
    40: "sha1",
    64: "sha256",
...
/**
 * Picks the column a hash names, going by how wide it is.
 *
 * No two digests recorded are the same length, so a caller need not say which
 * algorithm it holds. A width naming none of them is refused rather than
 * compared against every column and matched by nothing.
 */
export const digested = (hash: string) => {
    const kind = WIDTHS[hash.length];
    if (kind === undefined || !/^[0-9a-fA-F]+$/.test(hash)) return undefined;
    return schema.games_owned_rom[kind];
};

/** Answers a request whose hash names no recorded digest. */
export const unhashed = () =>
    fail(
        "invalid_query",
        "A hash must be 8, 32, 40, or 64 hexadecimal characters, for a " +
            "CRC-32, MD5, SHA-1, or SHA-256 digest.",
        [{ path: "/hash", message: "Not a recognized digest." }],
    );

/** Column declarations, in wire order. */
const FIELDS: Fields = {
    id: uuid("Unique identifier."),
    owned: uuid("Owned game item."),
    title: nullable(t.String(), "Title."),
    size: int("Size in bytes."),
    dumped: int("Dump date (Unix seconds)."),
};

/** Content digests, keyed by algorithm. */
const Hash = define(
    "Hash",
    t.Object(
        {
        ...
        { description: "Content digests (lowercase hex)." },
    ),
);
    ...
    t.Object(
        {
            ...
            present: bool("Presence in the store."),
        },
...
const Patch = t.Object(
    {
        title: t.Optional(nullable(t.String(), "Title.")),
        ...
        .get(
            ...
            async ({ query: args }) => {
                ...
                if (args.hash !== undefined) {
                    ...
                    digest = eq(column, args.hash);
                }

                // Apply filters
                const where = [
                    args.q ? like(rom.title, `%${args.q}%`) : undefined,
            ...
            },
            {
                query: t.Object({
                    ...
                    hash: t.Optional(
                        t.String({ description: "Filter by content digest." }),
                    ),
                ...
                ...operation({
                    ...
                    responses: { 200: json(t.Array(Rom)), 400: failed },
                }),
            },
        )
        .get(
        ...
        .patch(
            ...
            },
            {
                ...
                ...operation({
                    tag: TAG,
                    id: "modifyRom",
                    about: "Modify a ROM's title or position.",
                    auth: true,
                    responses: { 200: found, 404: failed },
                }),
            },
            ...
            async ({ params: args, request }) => {
                ...
                return new Response(file, {
                    headers: {
                        ...
                        "content-disposition": disposition(row.title ?? row.id),
                        etag,
                    },
                });
            },
